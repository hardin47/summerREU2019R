...
author: "Professor Jo Hardin, Pomona College Mathematics Department; Professor Dan Stoebel, Harvey Mudd College Biology Department; Madison Hobbs, Scripps College"
date: "6/27/2017"
output: pdf_document
---
...
6. Proportion of Reads From Each Feature Within Each Sample in the E. coli Data Set
...
By comparing aspects of our data, E. Coli, to the data in the paper on which you collaborated, "Hierarchical expression of genes controlled by the Bacillus subtilis global regulatory protein CodY," we seek to understand if what we see in the E. coli data is expected or reason for concern. Unless otherwise specified, E. Coli data used in this analysis includes all gene features (sens and antisens CDS, IGR, ncRNA, tRNA, rRNA) and will exclude the wild type condition because it does not have the rssB deletion consistent among all other samples.

We would greatly appreciate your perspective and input, and thank you for all of your help, time, and consideration on this project. 
...
ggplot(data = totalCounts.Ecoli, aes(x = cond.samps, y=total.count)) + geom_bar(stat = "identity") + ylab("Total Count") + xlab("Sample") + ggtitle("E. coli Total Counts by Sample")
```
```{r, echo = F, fig.height=4}

totalCounts.Bsub <- CodYcounts %>% select(WT1, WT2, F71Y1, F71Y2, R61K1, R61K2, R61H1, R61H2, null1, null2) %>% summarize_each(funs(sum)) %>%
  gather(cond.samps, total.count)
ggplot(data = totalCounts.Bsub, aes(x = cond.samps, y=total.count)) + geom_bar(stat = "identity") + ylab("Total Count") + xlab("Sample") + ggtitle("B. subtilis Total Counts by Sample")
```
