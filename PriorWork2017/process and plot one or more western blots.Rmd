...
- araConc - the concentration of arabinose added, a numerical value, possibly zero.
...
Plot the raw data with a log x-axis. This log scale means that zero arabinose can't be plotted. To solve this, I'll add 10^-6 to each arabinose concentration.
```{r}
...
A plot with just the levels used for this RNA-seq experiment. We used concentations of 0,1e-5, 5e-5, 1e-4, 1e-3, 2e-3, 5e-3.
```{r}
library("sfsmisc")

araColsUsed <- c(1,2,3,4,7,8,9) #These columns are the ones for the RpoS levels we used for the RNAseq experiment

rpoSmeanToPlot <- rpoSmean[2,araColsUsed] #Dropping conditions in columns 11 and 12 because there is only a single replicate. These are very high arabinose concentrations that I only did in a single experiment to explore the range. 
rpoSseToPlot <- rpoSse[2,araColsUsed]
